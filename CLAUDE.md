...
What lives on the server outside the repo:

- **Real datasets** — read-only under `/home/yiru/...` (the Datasets table above). External inputs,
  never copied into the repo.
- **scGEN offline dumps** — `~/scgen_dumps_*/<dataset>/` holding the export contract `{P}.h5ad` +
  `{P}_centers.npz` and the trained `{P}_seed.h5ad` (e.g. `~/scgen_dumps_edist/Saunders_b10/`, ~14 MB
  for 14 guides). Generated by `scripts/scgen/export_dumps.py` (concert env) → `run_scgen.py` (scgen
  env); point the notebook's `SCGEN_DUMP_DIR` at one of these. Not committed.
- **Single-slice symlink dirs** — e.g. `~/saunders_b10_slice/ -> Batch_10_Slice_0.h5mu`, used to pin
  ONE slice (the adapter otherwise loads the alphabetically-first `*.h5mu` in a dir). Not committed.
