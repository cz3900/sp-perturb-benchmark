...
The repo is **not pip-installed on the server** — the committed source is pushed to `~/sp-perturb-benchmark`.
GitHub is blocked on the cluster, so sync with `git archive HEAD | ssh zenglab 'tar -x -C ~/sp-perturb-benchmark'`
(+ `rsync -aqz ./.git/ zenglab:~/sp-perturb-benchmark/.git/` when history is needed), never `git pull`.
For an isolated re-run that must not disturb a concurrent task, push to a SEPARATE dir, e.g.
`~/sp-perturb-benchmark-edist` (what Part E of the retire-E work used).
...
consolidated: outputs under `~/sp-perturb-outputs/`, external model source under `~/sp-external/`,
input symlinks under `~/sp-inputs/`, ad-hoc runner scripts under `~/sp-scripts/`; the repo itself
stays at `~/sp-perturb-benchmark`):

- **Real datasets** — read-only under `/home/yiru/...` (the Datasets table above). External inputs,
  never copied into the repo.
- **scGEN offline dumps** — `~/sp-perturb-outputs/scgen/dumps_<tag>/<dataset>/` holding the export
  contract `{P}.h5ad` + `{P}_centers.npz` and the trained `{P}_seed.h5ad` (e.g.
  `~/sp-perturb-outputs/scgen/dumps_edist/Saunders_b10/`, ~14 MB for 14 guides). Generated by
...
- **SpatialProp dumps + working dir** — `~/sp-perturb-outputs/spatialprop/dumps_<dataset>/{P}.h5ad`
  (the `predicted_tempered` results) and `~/sp-perturb-outputs/spatialprop/run/` (export + the large
  regenerable GNN dataset cache). Not committed.
- **Single-slice symlink dirs** — e.g. `~/sp-inputs/saunders_b10_slice/ -> Batch_10_Slice_0.h5mu`,
