...
md("## 5. Run the baselines (D1 seed + D2 niche)\n"
   ...
   "cell is `model+learned`). `gain = e_null - e` > 0 beats the no-effect baseline, where "
   "**e_null is the aggregate same-type control niche** (the neighbourhood of the average "
   "unperturbed cell of that type) — NOT a matched or permutation null.")
...
md("## 9. Visual summary (the headline result)\n"
   ...
   "- **D1** — scGEN vs the TrivialSeed floor (`pcc_delta`, higher is better): is the conditional "
   "model beating the floor on the cell's own shift?\n"
   "- **D2** — niche gain vs the oracle upper bound (`gain = e_null - e`, >0 beats predicting "
   "'neighbours unchanged').\n"
   "- **Capability matrix** heatmap — each model scored only on the dims it covers ('—' = not "
   "covered).\n"
...
code("# ---- clean text dump of the final results ----\n"
