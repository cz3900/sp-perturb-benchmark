...
md("## 5. Run the baselines (seed + niche)\n"
   ...
   "- **seed** (`seed_logE`) — the seed/TrivialSeed log E; `seed_null_logE` is the **control-seed "
   "reference** (the same-type aggregate-control seed).\n"
   "- **niche** (`niche_logE`) — the deployable `model+learned` niche log E; `niche_null_logE` is the "
   "**no-effect reference** (the aggregate same-type control niche — the neighbourhood of the "
   "average unperturbed cell of that type), and `niche_oracle_logE` is the **leakage-free ceiling**.\n"
   "\n"
   "`null` and `oracle` are **reference values shown alongside, never subtracted** — a method beats "
   "no-effect when its `*_logE` sits **below** `*_null_logE`. PCC-delta (`seed_pcc`/`niche_pcc`) is kept "
...
code("res = run_benchmark(data, perturbations=EVAL, k=K_GRAPH, k_ref=K_REF,\n"
     ...
     "        seed_logE=_seed_logE(es.get('model', [])),          # seed (TrivialSeed) log E\n"
     "        seed_null_logE=_seed_logE(es.get('null', [])),      # seed control-seed reference\n"
     "        niche_logE=np.log(c['e']['model+learned']),         # niche (deployable) log E\n"
     "        niche_null_logE=np.log(c['e']['null']),             # niche no-effect reference\n"
     "        niche_oracle_logE=(np.log(c['e']['oracle']) if 'oracle' in c['e'] else float('nan')),  # niche ceiling ref\n"
     "        seed_pcc=s['pcc_delta'], niche_pcc=c['pcc']['model+learned'],  # supplementary (PCC-delta)\n"
     "        leak_ok=res['leakage_pass'][p],\n"
     "    ))\n"
     "# Sort by niche log E ascending (lower = better); null/oracle columns are references, not scores.\n"
     "base_df = pd.DataFrame(rows).sort_values('niche_logE', ascending=True).reset_index(drop=True)\n"
     "base_df")
...
code("from spbench.models.scgen_model import ScgenSeedModel\n"
     ...
     "    # scGEN seed as log(E-distance) (lower = better); pcc kept supplementary.\n"
     "    sc_df = pd.DataFrame([dict(perturbation=p, quadrant=QUADRANT_OF.get(p, mcj.UNKNOWN),\n"
     "                               scgen_seed_logE=_seed_logE(v.get('e_samples', {}).get('model', [])),\n"
     ...
     "    display(sc_df.sort_values('scgen_seed_logE', ascending=True).reset_index(drop=True))\n"
     "else:\n"
...
md("### Combined per-perturbation results table\n"
   "One row per perturbation, all in **log(E-distance), lower = better**: the baselines' **seed** "
   "(`seed_logE`, with `seed_null_logE` reference) and **niche** (`niche_logE`, with `niche_null_logE` and "
   "`niche_oracle_logE` references), plus scGEN's seed (`scgen_seed_logE`) when its dumps were found. "
...
code("def _capability_matrix(df):\n"
     ...
     "    cm = {'TrivialSeed (seed)': {'seed': float(np.nanmean(df['seed_logE'])), 'niche': np.nan, 'composition': np.nan},\n"
     "          'Gaussian/GCN (niche)': {'seed': np.nan,\n"
     "                                   'niche': float(np.nanmean(df['niche_logE'])), 'composition': np.nan},\n"
     "          'null (reference)': {'seed': float(np.nanmean(df['seed_null_logE'])),\n"
     "                               'niche': float(np.nanmean(df['niche_null_logE'])), 'composition': np.nan},\n"
     "          'oracle (reference)': {'seed': np.nan,\n"
     "                                 'niche': float(np.nanmean(df['niche_oracle_logE'])), 'composition': np.nan},\n"
     "          'niche-composition module': {'seed': np.nan, 'niche': np.nan, 'composition': 'simplex (built in §3)'}}\n"
     "    if 'scgen_seed_logE' in df.columns:\n"
     "        cm['scGEN (seed)'] = {'seed': float(np.nanmean(df['scgen_seed_logE'])), 'niche': np.nan, 'composition': np.nan}\n"
...
code("if HAVE_MC:\n"
     ...
     "        # seed gain = how far the seed log E sits below the control-seed reference (null) log E.\n"
     "        seed_ref = r['seed_null_logE']\n"
     "        seed_self = r['scgen_seed_logE'] if ('scgen_seed_logE' in base_df.columns\n"
     "                                             and pd.notna(r.get('scgen_seed_logE'))) else r['seed_logE']\n"
     "        seed_gain = float(seed_ref - seed_self)\n"
     "        # niche gain = how far the niche log E sits below the no-effect null log E.\n"
     "        niche_gain = float(r['niche_null_logE'] - r['niche_logE'])\n"
...
md("## 8. Bottom line — how to read it\n"
   ...
   "- **scGEN vs TrivialSeed on seed.** scGEN is a conditional model; its seed log E should sit "
   ...
   "- **niche.** `model+learned` log E **below `niche_null_logE`** beats predicting 'the "
   "neighbours did not change', and that advantage should be concentrated in the **Niche/Both** "
   "quadrants, not in Inert; `niche_oracle_logE` is the leakage-free floor.\n"
...
code("# All numbers are log(E-distance), LOWER = better; null/oracle are references (never subtracted).\n"
     "has_scgen = 'scgen_seed_logE' in base_df.columns\n"
     ...
     "niche_mean = float(np.nanmean(base_df['niche_logE'])); niche_null = float(np.nanmean(base_df['niche_null_logE']))\n"
     "n_niche_win = int((base_df['niche_logE'] < base_df['niche_null_logE']).sum())\n"
     "print(f'\\nniche: mean log E {niche_mean:.3f}  vs  null {niche_null:.3f}  (lower = better)')\n"
     "print(f'    {n_niche_win}/{len(base_df)} guides beat no-effect (niche log E below null).')\n"
     "if has_scgen:\n"
     "    sc_mean = float(np.nanmean(base_df['scgen_seed_logE'])); tr_mean = float(np.nanmean(base_df['seed_logE']))\n"
     "    better = base_df['scgen_seed_logE'] < base_df['seed_logE']\n"
     "    print(f'seed: scGEN mean log E {sc_mean:.3f}  vs  TrivialSeed {tr_mean:.3f}  (lower = better);'\n"
...
md("## 9. Visual summary (the headline result)\n"
   ...
   "rows. The per-method seed and niche log-E box plots are the **§5b** figure "
...
code("# ---- clean text dump of the final results (all log(E-distance), lower = better) ----\n"
     ...
     "has_scgen = 'scgen_seed_logE' in base_df.columns\n"
     "cols = ['perturbation', 'quadrant', 'seed_logE']\n"
     "if has_scgen:\n"
     "    cols += ['scgen_seed_logE']\n"
     "cols += ['niche_logE', 'niche_null_logE', 'niche_oracle_logE']\n"
     "print('=' * 72)\n"
     ...
     "if has_scgen:\n"
     "    sc_mean = float(np.nanmean(base_df['scgen_seed_logE'])); tr_mean = float(np.nanmean(base_df['seed_logE']))\n"
     "    n_win = int((base_df['scgen_seed_logE'] < base_df['seed_logE']).sum())\n"
     "    print(f'\\nseed: scGEN mean log E {sc_mean:.3f}  vs  TrivialSeed mean log E {tr_mean:.3f}'\n"
     "          f'   (lower wins; scGEN lower on {n_win}/{len(base_df)} guides)')\n"
     "n_niche = int((base_df['niche_logE'] < base_df['niche_null_logE']).sum())\n"
     "niche_mean = float(np.nanmean(base_df['niche_logE'])); niche_null = float(np.nanmean(base_df['niche_null_logE']))\n"
     "print(f'niche: mean log E {niche_mean:.3f}  vs  null {niche_null:.3f}  '\n"
